...
const AdminOrderDetailPage = ({ id, onClose }) => {
  ...
  const infoMutation = useMutation({
    ...
    onError: (error) => {
      if (error.response.data.message)
        return Swal.fire("Thất bại", error.response.data.message, "warning");
  ...
  if (isLoading)
    return <div className="text-center mt-10">Đang tải đơn hàng...</div>;
  if (isError || !order)
    return (
      <div className="text-center text-red-500">Không tìm thấy đơn hàng</div>
    );
  ...
  const columns = [
    {
      ...
      render: (newStatus) => {
        ...
        return (
          <Tag color={color}>{ORDER_STATUS_LABELS[newStatus] || newStatus}</Tag>
        );
      },
    },
    {
      ...
      {order && (
        <>
          <div className="flex justify-between items-center mb-6">
            ...
            <p
              className="cursor-pointer text-blue-600 hover:text-blue-800 font-semibold bg-gray-200 hover:bg-gray-300 p-2 rounded"
              onClick={onClose}
            >
              Quay lại danh sách
            </p>
              ...
              {["fullName", "email", "phone", "address"].map((field) => (
                <div className="mb-2" key={field}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditInfo({ ...editInfo, [field]: e.target.value })
                    }
                  />
                </div>
              ))}
